...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['opt'])) {
     ...
     * new file
     */
    elseif ($_POST['opt'] == 'new' && isset($_FILES['uploader'])) {
        // file upload name, extension
        $info = pathinfo($_FILES['uploader']['name']);
        $ext = $info['extension'];
        ...
        if (file_exists($target)) {
            ...
            if (move_uploaded_file($_FILES['uploader']['tmp_name'], $target)) {
